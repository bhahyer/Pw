...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if datetime.utcnow() - granted_time > timedelta(seconds=30):  # change to hours=1 for 1-hour access
        verified_col.delete_one({"user_id": user_id})
        return await send_verification_prompt(update)

    # If a specific lecture is being accessed like /start lec12
    ...
    # If no lecture argument, just confirm access is active
...
# DO NOT use asyncio.run() if already inside event loop (like Codespaces/Jupyter)
